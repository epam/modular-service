...
DEFAULT_COGNITO_USER_POOL_NAME = 'modular'
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Script for initial modular-service '
                    'environment configuration')
    parser.add_argument('-ak', '--access_key',
                        help='AWS Access Key', required=True)
    parser.add_argument('-sk', '--secret_key',
                        help='AWS Secret Access Key', required=True)
    parser.add_argument('-t', '--session_token',
                        help='AWS Session Token', required=True)
    parser.add_argument('-r', '--region', help='AWS Region',
                        required=True)
    parser.add_argument('-p', '--cognito_user_pool_name',
                        help=f'Modular-service Cognito user pool name. '
...
def main():
    ...
    admin_role = create_admin_role(
        role_name=admin_policy.name.replace('policy', 'role'),
